---
title: 'Habilitación que se adapta: convertir modelos de Workfront en programas de formación reales'
description: Aprenda a convertir los modelos de Workfront en programas de formación prácticos para escalar la habilitación de forma eficaz.
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3496
last-substantial-update: 2026-03-26T00:00:00Z
jira: KT-20608
source-git-commit: c350251d880f9e1ea4de6de51e1139894c9a3b47
workflow-type: tm+mt
source-wordcount: '195'
ht-degree: 2%

---


# Habilitación que se adapta: convertir modelos de Workfront en programas de formación reales

A medida que las organizaciones crecen, también lo hace la complejidad de la incorporación, la formación y la asistencia a los usuarios. Lo que funciona para un pequeño equipo piloto a menudo se descompone a escala. Es allí donde los modelos de Adobe Workfront y las plantillas de proyecto pueden proporcionar estructura, coherencia y velocidad. En esta sesión, exploramos cómo Workfront puede impulsar el motor para la formación y la habilitación, con tecnología de modelos.

>[!VIDEO](https://video.tv.adobe.com/v/3481826/?learn=on&enablevpops)

Si se ha perdido la sesión, no se preocupe. Revise la [presentación](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/022626+-+Enablement+that+Scales+-+Training+and+Blueprints+slides.pdf) y vea la grabación bajo demanda para continuar.

¿Busca más recursos del evento? ¡Mira la [publicación de la comunidad Experience League](https://experienceleaguecommunities.adobe.com/adobe-workfront-23/event-follow-up-enablement-that-scales-turning-workfront-blueprints-into-real-world-training-programs-february-26-2026-248830)!

Se agregan nuevos eventos cada mes, así que asegúrese de consultar la [página Eventos de Experience League](https://experienceleague.adobe.com/es/events) para ver las sesiones más recientes.

